function Odes = SimplifiedLinearMaxwellFoundationContactOdes(x, M, region, MOld, modelParams)
...
uHatOld = modelParams.uHat;
    ...
    function dMdS = ContactEqns(x, M, region)
        ...
        if (length(gamma) > 1)
            gammaInterp = interparc(x/MOld.x(end), MOld.x, gamma, 'linear');
            gamma = gammaInterp(:, 2)';
        end
        
        if (length(K) > 1)
            KInterp = interparc(x/MOld.x(end), MOld.x, K, 'linear');
            K = KInterp(:,2)';
        end
        
        if (length(Es) > 1)
            EsInterp = interparc(x/MOld.x(end), MOld.x, Es, 'linear');
            Es = EsInterp(:,2)';
        end
        
        if (length(Eb) > 1)
            EbInterp = interparc(x/MOld.x(end), MOld.x, Eb, 'linear');
            Eb = EbInterp(:,2)';
        end
        
        if (length(YOld) > 1)
            YOldInterp = interparc(x/MOld.x(end), MOld.x, YOld, 'linear');
            YOld = YOldInterp(:,2)';        
        end
        
        if (length(mOld) > 1)
            mOldInterp = interparc(x/MOld.x(end), MOld.x, mOld, 'linear');
            mOld = mOldInterp(:,2)';        
        end
        
        if (length(POld) > 1)
            POldInterp = interparc(x/MOld.x(end), MOld.x, POld, 'linear');
            POld = POldInterp(:,2)';
        end
        
        if (length(uHatOld) > 1)
            uHatOldInterp = interparc(x/MOld.x(end), MOld.x, uHatOld, 'linear');
